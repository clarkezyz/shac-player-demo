...
class SpatialAudioEngine {
    ...
    async loadSHAC(shacData) {
        ...
        for (const [layerName, layerData] of layers) {
            ...
            // Store source position for movement presets
    ...
    async play() {
        ...
        for (const [name, layer] of this.layers) {
            ...
            // Add ended handler to manually restart for demo looping
            sourceNode.onended = () => {
                if (this.isPlaying) {
                    // Restart this layer immediately
                    const newSourceNode = this.audioContext.createBufferSource();
                    newSourceNode.buffer = layer.audioBuffer;
                    newSourceNode.loop = true;
                    newSourceNode.connect(layer.pannerNode);
                    newSourceNode.start(0, 0); // Always start from beginning for demo loop
                    layer.sourceNode = newSourceNode;
                    
                    // Re-add ended handler
                    newSourceNode.onended = sourceNode.onended;
                }
            };
