...
class SHACPlayer {
    ...
    async init() {
        try {
            // Show loading with demo message
            this.controls.showLoading('Loading Revolutionary Interactive Spatial Audio...', 'Initializing...');
            
            // Initialize audio engine
            await this.audioEngine.init();
            
            // Auto-load the demo SHAC file
            await this.loadDemoFile();
            ...
            // Setup info buttons (remove file input setup)
            this.setupInfoButtons();
            
            // Setup ESC key for gallery
            this.setupEscapeKey();
            ...
            this.controls.updateTrackInfo(
                'SHAC Demo',
                'Move to explore the space'
            );
            
            // Add ESC to pause hint
            this.addPauseHint();
            
            // Hide play/pause button for demo (it should just loop)
    ...
    async loadDemoFile() {
        try {
            this.controls.updateLoadingDetails('Loading revolutionary interactive spatial audio experience...');
            
            // Fetch the demo SHAC file 
            const response = await fetch('landing_demo.shac');
            if (!response.ok) {
            ...
            // Read the response as a stream to show progress
            ...
            while(true) {
                ...
                if (total) {
                    ...
                    this.controls.updateLoadingDetails(`Loading interactive spatial audio: ${percentComplete}%`);
                }
            }
            ...
            this.controls.updateLoadingDetails('Decoding SHAC format...');
            
            // Decode SHAC
            const decoder = new PythonSHACDecoder();
            const shacData = await decoder.decode(allChunks.buffer);
            
            this.controls.updateLoadingDetails('Initializing interactive spatial engine...');
            
            // Load into audio engine
            await this.audioEngine.loadSHAC(shacData);
            this.currentSHAC = shacData;
            
            // Update visualizer with layer data
            this.visualizer.updateData(
                this.audioEngine.getLayerInfo(),
                this.audioEngine.listenerPosition,
                this.audioEngine.listenerRotation
            );
            
            this.controls.updateLoadingDetails('Ready to experience interactive spatial audio!');
    ...
    setupEscapeKey() {
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape') {
                // Show gallery dialog (doesn't pause audio - keeps looping)
                this.showPauseOverlay();
            }
        });
        
        // Setup gallery dialog close button
        const galleryDialog = document.getElementById('gallery-dialog');
        if (galleryDialog) {
            const closeBtn = galleryDialog.querySelector('.close-btn');
            if (closeBtn) {
    ...
    startUpdateLoop() {
        ...
        this.updateInterval = setInterval(() => {
            if (this.audioEngine.isPlaying) {
                ...
                // Auto-restart for demo looping (don't auto-stop)
                if (currentTime >= duration) {
                    this.audioEngine.replay();
                }
            }
        }, 100);
    }
    ...
    /**
     * Add bouncing ESC to pause hint
     */
    addPauseHint() {
        ...
        if (subtitle) {
            ...
            pauseHint.innerHTML = '[esc] for gallery';
            subtitle.appendChild(pauseHint);
        }
    }
    
    /**
     * Hide play/pause button for demo (should just loop)
     */
    hidePlayPauseButton() {
    ...
    /**
     * Show pause overlay with gallery option
     */
    showPauseOverlay() {
        const pauseOverlay = document.createElement('div');
        pauseOverlay.id = 'pause-overlay';
        pauseOverlay.className = 'overlay show';
        pauseOverlay.innerHTML = `
            <div class="overlay-content" style="text-align: center; max-width: 500px;">
                <h2 style="color: var(--primary-color); margin-bottom: 20px;">✨ SHAC Magic Awaits</h2>
                <p style="font-size: 1.1rem; line-height: 1.6; margin-bottom: 30px; color: var(--text-primary);">
                    Interested in hearing more SHAC magic? Check out our Gallery for a curated selection of interactive spatial audio experiences.
                </p>
                <div style="margin-bottom: 25px;">
                    <button class="primary-btn gallery-btn" onclick="window.open('https://zyzd.cc/shac/gallery/', '_blank')" 
                ...
                <button class="continue-btn" onclick="document.getElementById('pause-overlay').remove()" 
                        style="background: transparent; border: 2px solid var(--text-secondary); 
                               color: var(--text-secondary); padding: 12px 24px; 
                               border-radius: 6px; font-size: 0.95rem; cursor: pointer;
                               transition: all 0.3s ease;">
                    Continue Exploring This Demo
                </button>
            </div>
        `;
