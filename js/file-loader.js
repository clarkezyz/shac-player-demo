...
class UniversalFileLoader {
    constructor() {
        this.supportedFormats = {
            '.shac': 'Spatial Harmonic Audio Codec'
        };
    }
    ...
    async decode(arrayBuffer, extension) {
        // Ensure we have the SHAC decoder available
        if (extension === '.shac' && typeof SHACDecoder === 'undefined') {
            throw new Error('SHAC decoder not loaded. Please ensure shac-decoder.js is included.');
        }
        
        try {
            console.log('Decoding SHAC format...');
    ...
    getFileExtension(filename) {
        const match = filename.toLowerCase().match(/\.(shac)$/);
        if (!match) {
            throw new Error(`File must have .shac extension`);
        }
        return match[0];
    }
